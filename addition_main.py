...
def run_training(train_data, val_data, experiment_name):
    ...
    for step in range(config.TRAIN_STEPS):
        ...
        if step%config.EVAL_INTERVAL==0 or step==config.TRAIN_STEPS-1:
            ...
            print(f"实验{experiment_name} 步数{step}/{config.TRAIN_STEPS}|训练损失:{loss.item():.4f}|验证损失:{val_loss_avg:.4f}|耗时:{time.time()-start_time:.2f}s")
            start_time=time.time()
...
def run_evaluation(model, tokenizer, test_digit_pairs, num_test_cases=200):
    ...
    for d1, d2 in test_digit_pairs:
        ...
        for i in range(num_test_cases):
            ...
            model_answer=tokenizer.decode(generated_tokens)[len(prompt):].strip()
